...
for col, step in enumerate([0.25, 0.0]):
    ...
    axes[1, col].plot(tt, d1_true, "-", color="black", lw=1.8, label="True $d_1$")
    axes[1, col].plot(tt, d1_hat, "--", color="purple", lw=2.0, label="KF $d_1$")
    axes[1, col].fill_between(tt, d1_hat - sd1, d1_hat + sd1, color="purple", alpha=0.18)

    axes[1, col].plot(tt, d2_true, "-", color="black", lw=1.8, label="True $d_2$")
    axes[1, col].plot(tt, d2_hat, "--", color="magenta", lw=2.0, label="KF $d_2$")
    axes[1, col].fill_between(tt, d2_hat - sd2, d2_hat + sd2, color="magenta", alpha=0.18)

    axes[1, col].set_ylabel("Flow [m³/s]")
    ...
    axes[2, col].plot(tt, y1_true_abs, ".", color="black", ms=2, alpha=0.9, label="True $z_1$")
    axes[2, col].plot(tt, y1_hat_abs, "--", color="dodgerblue", lw=2.0, label="KF $z_1$")
    axes[2, col].fill_between(tt, y1_hat_abs - 2 * sy[:, 0], y1_hat_abs + 2 * sy[:, 0], color="dodgerblue", alpha=0.15)

    axes[2, col].plot(tt, y2_true_abs, ".", color="black", ms=2, alpha=0.9, label="True $z_2$")
    axes[2, col].plot(tt, y2_hat_abs, "--", color="tomato", lw=2.0, label="KF $z_2$")
    axes[2, col].fill_between(tt, y2_hat_abs - 2 * sy[:, 1], y2_hat_abs + 2 * sy[:, 1], color="tomato", alpha=0.15)

    axes[2, col].set_ylabel("Height [m]")
    ...
    axes[1, col].set_title("Disturbance input")
    axes[2, col].set_title("Output")
...
plt.tight_layout()
plt.savefig("figures/Problem6/Problem4_KF_step_vs_nostep.png", dpi=200)
plt.close()
