...
T,X,Y,Z = sim.openloop(sys.f_modified,sys.g,sys.h, t0, tf, x0, U, D, p)
...
sigma1 = np.sqrt(0.1)
sigma2 = np.sqrt(0.1)
F3 = np.random.normal(mu1,sigma1,size=tf)
F4 = np.random.normal(mu2,sigma2,size=tf) 
D[0, :] = F3
D[1, :] = F4 
...
#%% Simulate deterministic open-loop (Using SDE)
...
# Plotting example
fig, axes = plt.subplots(2, 1, figsize=(8, 12), sharex=True)
axes[0].plot(T.ravel(), X[0, :]/(rho*A1), label='$H_1$')
axes[0].plot(T.ravel(), X[1, :]/(rho*A2), label='$H_2$')
axes[0].plot(T.ravel(), X[2, :]/(rho*A3), label='$H_3$')
axes[0].plot(T.ravel(), X[3, :]/(rho*A4), label='$H_4$')
axes[0].set_xlabel('Time [s]')
axes[0].set_ylabel('Height [m]')
axes[0].legend()
axes[0].set_title('Open loop simulation of SDE model')
axes[1].step(T.ravel(), U[0, :], label='$F_1$')
axes[1].step(T.ravel(), U[1, :], label='$F_2$')
axes[1].step(T.ravel(), D[0, :], label='$F_3$')
axes[1].step(T.ravel(), D[1, :], label='$F_4$')
